...
## This code builds the transportation sector dataset used to produce results in
## the manuscript. #############################################################
################################################################################
...
# Download the CO2 emissions dataset by sector
url <- "https://www.eia.gov/state/seds/sep_sum/html/xls/co2_sector.xlsx"
destfile <- "co2_sector.xlsx"
download.file(url, destfile, mode = "wb")

## Load the "Total" CO2 emissions sheet and skip the first row, which is empty
data <- read_excel(destfile, sheet = 5, skip = 1)
head(data)
...
#### SAVE
save(final_data,n_until2015,file=paste0(project.dir,"/final_data_transportation.RData"))
