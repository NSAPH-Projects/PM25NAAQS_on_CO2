...
# 1. Read data
naturalgasshifting<-read.table("increase_naturalgas.txt")
load(paste(project.dir,"/main_analysis.RData",sep=""))

# 2. Meta-analysis for each period

df2009$naturalgasshifting<-ifelse(df2009$State%in%naturalgasshifting$State[which(naturalgasshifting$naturalgasshifting2009==1)],1,0)
df2012$naturalgasshifting<-ifelse(df2012$State%in%naturalgasshifting$State[which(naturalgasshifting$naturalgasshifting2012==1)],1,0)
df2014$naturalgasshifting<-ifelse(df2014$State%in%naturalgasshifting$State[which(naturalgasshifting$naturalgasshifting2014==1)],1,0)

reg_2009_fr <- rma(yi = cum_effect, sei = sd,  method = "REML", mods = ~ as.factor(naturalgasshifting), 
                   data = df2009)
s_reg2009_fr <- summary(reg_2009_fr)

reg_2009_rel_fr <- rma(yi = cum_effect_rel, sei = sd_rel,  method = "REML", mods = ~ as.factor(naturalgasshifting), 
                   data = df2009)
s_reg2009_rel_fr <- summary(reg_2009_rel_fr)

reg_2012_fr <- rma(yi = cum_effect, sei = sd,  method = "REML", mods = ~ as.factor(naturalgasshifting), 
                   data = df2012)
s_reg2012_fr <- summary(reg_2012_fr)

reg_2012_rel_fr <- rma(yi = cum_effect_rel, sei = sd_rel,  method = "REML", mods = ~ as.factor(naturalgasshifting), 
                       data = df2012)
s_reg2012_rel_fr <- summary(reg_2012_rel_fr)

reg_2014_fr <- rma(yi = cum_effect, sei = sd,  method = "REML", mods = ~ as.factor(naturalgasshifting), 
                   data = df2014)
s_reg2014_fr <- summary(reg_2014_fr)

reg_2014_rel_fr <- rma(yi = cum_effect_rel, sei = sd_rel,  method = "REML", mods = ~ as.factor(naturalgasshifting), 
                       data = df2014)
s_reg2014_rel_fr <- summary(reg_2014_rel_fr)

saveRDS(s_res2009, file = paste0(project.dir,"/main_results/meta_analysis_20052009_naturalgasshifting.rds"))
saveRDS(s_res2012, file = paste0(project.dir,"/main_results/meta_analysis_20052012_naturalgasshifting.rds"))
saveRDS(s_res2014, file = paste0(project.dir,"/main_results/meta_analysis_20052014_naturalgasshifting.rds"))
saveRDS(s_res2009_rel, file = paste0(project.dir,"/main_results/meta_analysis_rel_20052009_naturalgasshiftingg.rds"))
saveRDS(s_res2012_rel, file = paste0(project.dir,"/main_results/meta_analysis_rel_20052012_naturalgasshifting.rds"))
saveRDS(s_res2014_rel, file = paste0(project.dir,"/main_results/meta_analysis_rel_20052014_naturalgasshifting.rds"))
...
df_naturalgasshifting <- left_join(df_reg, naturalgasshifting[, c(1,4)], by = "State")
df_naturalgasshifting$naturalgasshifting <- as.factor(df_naturalgasshifting$naturalgasshifting2014)
levels(df_naturalgasshifting$naturalgasshifting) <- c("No effect", "Significant effect")
...
extract_group_meta <- function(m.rma, year_label) {
  ...
  eff1 <- eff0 + cf[grep("naturalgasshifting", rownames(cf)), "estimate"]
  # This is a conservative (over-)estimate of SE; for display only
  se1  <- sqrt(se0^2 + cf[grep("naturalgasshifting", rownames(cf)), "se"]^2)
  ci1  <- c(
    cf["intrcpt", "estimate"] + cf[grep("naturalgasshifting", rownames(cf)), "ci.lb"],
    cf["intrcpt", "estimate"] + cf[grep("naturalgasshifting", rownames(cf)), "ci.ub"]
  )
  data.frame(
    State = c("Group mean1","Group mean2"),
    naturalgasshifting = c("No effect", "Significant effect"),
    mean = c(eff0, eff1),
    lower = c(ci0[1], ci1[1]),
    upper = c(ci0[2], ci1[2]),
    Year = year_label,
...
summary_df <- df_naturalgasshifting %>%
  mutate(
    lower = cum_effect_rel - 1.96 * sd_rel,
    upper = cum_effect_rel + 1.96 * sd_rel
  ) %>%
  select(State, mean = cum_effect_rel, lower, upper, Year, naturalgasshifting)
...
plot_data$panel_style <- ifelse(
  plot_data$naturalgasshifting == "No effect",
  "No effect",
  "Significant effect"
)
...
plot_data$naturalgasshifting_panel <- ifelse(
  plot_data$State %in% c("Group mean1","Group mean2"),
  "Average effects",
  as.character(plot_data$naturalgasshifting)
)

plot_data$naturalgasshifting_panel <- factor(
  plot_data$naturalgasshifting_panel,
  levels = c(
    "No effect",
    "Significant effect",
    "Average effects"
