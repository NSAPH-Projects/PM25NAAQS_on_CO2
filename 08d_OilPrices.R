...
## This code reproduces results in Table S9 (line "Oil Price") in the ##########
...
data<-final_data[which(final_data$State=="AL"),]

ce <- CausalArima(y = ts(data$Oil_Price, start = 1, frequency = 1), 
                    dates = dates, 
  ...
  avg_distr_0509 <- colMeans(data$Oil_Price[data$Year>=2005&data$Year<=2009] - 
  ...
  avg_distr_0512 <- colMeans(data$Oil_Price[data$Year>=2005&data$Year<=2012] - 
  ...
  avg_distr_0514 <- colMeans(data$Oil_Price[data$Year>=2005&data$Year<=2014] - 
  ...
  effect_avg_0509<-cbind(ce_avg_0509,
                               counter_0505,
                               ce_avg_0509_rel,
                               t(ce_avg_0509_int),
  ...
  effect_avg_0512<-cbind(ce_avg_0512,
                               counter_0512,
                               ce_avg_0512_rel,
                               t(ce_avg_0512_int),
  ...
  effect_avg_0514<-cbind(ce_avg_0514,
                               counter_0514,
                               ce_avg_0514_rel,
                               t(ce_avg_0514_int),
...
################################ 2009 ##########################################
df2009<-data.frame(avg_effect=round(effect_avg_0509[,1],3),
...
df2012<-data.frame(avg_effect=round(effect_avg_0512[,1],3),
...
df2014<-data.frame(avg_effect=round(effect_avg_0514[,1],3),
...
tableS9<-rbind(cbind(paste0(round(df2009$avg_effect,3),
                           ifelse(df2009$bi_pval<0.001,"***",
                                  ifelse(df2009$bi_pval<0.01,"**",
                                         ifelse(df2009$bi_pval<0.05,"*","")))),
                    paste0(round(df2009$avg_effect_rel*100,3),
                           ifelse(df2009$bi_pval<0.001,"***",
                                  ifelse(df2009$bi_pval<0.01,"**",
                                         ifelse(df2009$bi_pval<0.05,"*","")))),
                    paste0(round(df2012$avg_effect,3),
                           ifelse(df2012$bi_pval<0.001,"***",
                                  ifelse(df2012$bi_pval<0.01,"**",
                                         ifelse(df2012$bi_pval<0.05,"*","")))),
                    paste0(round(df2012$avg_effect_rel*100,3),
                           ifelse(df2012$bi_pval<0.001,"***",
                                  ifelse(df2012$bi_pval<0.01,"**",
                                         ifelse(df2012$bi_pval<0.05,"*","")))),
                    paste0(round(df2014$avg_effect,3),
                           ifelse(df2014$bi_pval<0.001,"***",
                                  ifelse(df2014$bi_pval<0.01,"**",
                                         ifelse(df2014$bi_pval<0.05,"*","")))),
                    paste0(round(df2014$avg_effect_rel*100,3),
                           ifelse(df2014$bi_pval<0.001,"***",
                                  ifelse(df2014$bi_pval<0.01,"**",
                                         ifelse(df2014$bi_pval<0.05,"*",""))))),
              cbind(round(df2009$sd,3),round(df2009$sd_rel,3),
                    round(df2012$sd,3),round(df2012$sd_rel,3),
                    round(df2014$sd,3),round(df2014$sd_rel,3)))
colnames(tableS9)<-c("2005-2009","2005-2009 rel",
                    "2005-2012","2005-2012 rel",
                    "2005-2014","2005-2014 rel")
rownames(tableS9)<-c("effect","S.E.")

saveRDS(tableS9, file = paste0(project.dir,"/tables/TableS9_OilPrices.rds"))
