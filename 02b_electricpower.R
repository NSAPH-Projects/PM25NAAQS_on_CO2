...
## This code reproduces results of the analysis by sector reported in text, and 
## Figure S2 and Table S2 (line "Electric power") and Table S3 in the ##########
...
# upload data
load(paste0(project.dir,"/final_data_electricpower.RData"))
...
# remove Vermont because it has 0 CO2 emissions in the power sector
df2009_m <- df2009[-which(df2009$State=="VT"),]
df2012_m <- df2012[-which(df2012$State=="VT"),]
df2014_m <- df2014[-which(df2014$State=="VT"),]

res_2009 <- rma(yi = cum_effect, sei = sd, data = df2009_m, method = "REML")
s_res2009 <- summary(res_2009)

res_2012 <- rma(yi = cum_effect, sei = sd, data = df2012_m, method = "REML")
s_res2012 <- summary(res_2012)

res_2014 <- rma(yi = cum_effect, sei = sd, data = df2014_m, method = "REML")
s_res2014 <- summary(res_2014)

res_2009_rel <- rma(yi = cum_effect_rel, sei = sd_rel, data = df2009_m, method = "REML")
s_res2009_rel <-summary(res_2009_rel)

res_2012_rel <- rma(yi = cum_effect_rel, sei = sd_rel, data = df2012_m, method = "REML")
s_res2012_rel <- summary(res_2012_rel)

res_2014_rel <- rma(yi = cum_effect_rel, sei = sd_rel, data = df2014_m, method = "REML")
...
saveRDS(s_res2009, file = paste0(project.dir,"/main_results/meta_analysis_20052009_electricpower.rds"))
saveRDS(s_res2012, file = paste0(project.dir,"/main_results/meta_analysis_20052012_electricpower.rds"))
saveRDS(s_res2014, file = paste0(project.dir,"/main_results/meta_analysis_20052014_electricpower.rds"))
...
print(doc, target = paste0(project.dir,"/tables/TableS3.docx"))
...
jpeg(file = paste0(project.dir,"/plots/FigS2.jpeg"),
